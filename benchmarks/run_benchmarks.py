...
def peak_memory() -> tuple[float, str]:
    """
    Return ``(peak megabytes, source)``.

    The cgroup high-water mark is preferred, because it is the only reading
    that covers the whole process tree at once. It is readable in a container;
    on a bare CI runner it is not, and the fallback then takes the larger of
    our own peak and the largest resolver subprocess we waited for. That is
    the peak of a single process rather than of the tree — but ignoring
    children entirely understates Rust by an order of magnitude, since almost
    all of its memory belongs to ``rust-analyzer scip``.
    """
    ...
    return (maxrss * factor) / (1024 * 1024), "getrusage.max"
...
def _mem_scope(mem_src: str) -> str:
    """Say honestly what the peak-memory reading actually covers."""
    if mem_src.startswith("cgroup"):
        return "whole process tree, including resolver subprocesses"
    return (
        "largest single process — ours or a resolver subprocess; no cgroup "
        "counter was readable, so this is not the tree total"
    )
...
def render_table(results: list[BenchResult], version: str) -> str:
    ...
    lines.extend(
        [
            "",
            (
                f"<sub>Peak RSS measured via `{mem_src}` ({_mem_scope(mem_src)}). "
